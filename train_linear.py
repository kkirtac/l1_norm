"""Trains the model having dense layers and in-between relu.
...
from model.linear_model import L1NormLinear

results = []

model_names = ["L1NormLinear"]
...
for num_sample, max_len, inp_range in product(num_samples, max_length, input_range):
    ...
    train_loader = DataLoader(train_dataset, batch_size=32, num_workers=4, shuffle=True, collate_fn=collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=32, num_workers=4, shuffle=False, collate_fn=collate_fn)
    test_loader = DataLoader(val_dataset, batch_size=32, num_workers=4, shuffle=False, collate_fn=collate_fn)
    ...
    for model_name, hid_size in product(model_names, hidden_sizes):
        ...
        df_result.to_csv("output/results_linear.csv", index=False)
